# Create the ligand receptors tables for KO
...
load(paste0(gene2cell_association_dir, "gene2cellAssociation-ko.RData"))
...
activeAtDaysCell <- lapply(cells, function(cell) {
  cellCB:::getActiveAtDaysFromRData(paste0(resultsDir, "/", cell, "-ko.RData"), paste0(cell, "_ko"))
})
names(activeAtDaysCell) <- cells

genesBehavioursCell <- lapply(cells, function(cell) {
  extractGenesBehavious(activeAtDaysCell[[cell]], NULL, gene2cellAssociation[[cell]])
})
names(genesBehavioursCell) <- cells

# Let our cell population be fap, we came up with a list of time points and genes that are activated in fap. A list of time point were we stored genes that are active in total muscle but that belong also to fap. And a list of genes that constitutively are expressed by fap.
...
# We have collected some list of receptor/ligand activation. Now we can create the timeSpecific network.
...
write.table(ligandMatrix, file=paste0(tableDir ,"/", "ligandDescription-ko.txt"), sep="\t", quote=F, row.names=F)

receptorMatrix <- do.call(rbind, receptorsTimingCell)
row.names(receptorMatrix) <- NULL
write.table(receptorMatrix, file=paste0(tableDir, "/", "receptorDescription-ko.txt"), sep="\t", quote=F, row.names=F)

sifTables.ko <- sifTables
save(sifTables.ko, receptorMatrix, ligandMatrix, file=paste0(tableDir, '/sifTables.ko.RData') )
